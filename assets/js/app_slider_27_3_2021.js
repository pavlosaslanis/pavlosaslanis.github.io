...
//  }); 
  ...
  markers_alla_tobak.eachLayer(function (layer) {
    if (map.hasLayer(markers_alla_tobak)) {
      if (map.getBounds().contains(layer.getLatLng())) {
        $("#feature-list tbody").append('<tr class="feature-row" id="' + L.stamp(layer) + '" lat="' + layer.getLatLng().lat + '" lng="' + layer.getLatLng().lng + '"><td style="vertical-align: middle;"><img width="16" height="18" src="assets/img/alla_bolag.png"></td><td class="feature-name">' + layer.feature.properties.name + '</td><td style="vertical-align: middle;"><i class="fa fa-chevron-right pull-right"></i></td></tr>');
      }
    }
  }); 
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
var parameters = L.Util.extend(defaultParameters21);
...
var defaultParameters17 = {
    ...
    version: '1.0.0',
    request: 'GetFeature',
    typeName: 'split_shops:shops_17_18',
    maxFeatures: 3500,
    outputFormat: 'text/javascript'
   , format_options: 'callback: getJson',
    srsName:'EPSG:4326'
...
var owsrootUrl = 'http://localhost:8081/geoserver/split_shops/ows';
var parameters = L.Util.extend(defaultParameters17);
var URL = owsrootUrl + L.Util.getParamString(parameters);
...
var markers17 = new L.MarkerClusterGroup({ 
    iconCreateFunction: function (cluster) {
        var markers = cluster.getAllChildMarkers();
        var html = '<div class="circleRed">' + markers.length + '</div>';
        return L.divIcon({ html: html, className: 'mycluster', iconSize: L.point(32, 32) });
    },
    spiderfyOnMaxZoom: true, showCoverageOnHover: true, zoomToBoundsOnClick: true 
});
...
var owsrootUrl = 'http://localhost:8081/geoserver/split_shops/ows';
var parameters = L.Util.extend(defaultParameters_alla_tobak);
var URL = owsrootUrl + L.Util.getParamString(parameters);
...
var markers_alla_tobak = new L.MarkerClusterGroup({ 
    iconCreateFunction: function (cluster) {
        var markers = cluster.getAllChildMarkers();
        var html = '<div class="circleOrange">' + markers.length + '</div>';
        return L.divIcon({ html: html, className: 'mycluster', iconSize: L.point(32, 32) });
    },
    spiderfyOnMaxZoom: true, showCoverageOnHover: true, zoomToBoundsOnClick: true 
});
...
/// Load shops when loading the map...
shops_groups21.addTo(map)

var groupedOverlays = {
  "Customers": {
    "<img src='assets/img/theater.png' width='15' height='19'>&nbsp;Shops (24/3/2020)": shops_groups21,
    "<img src='assets/img/shops17.png' width='15' height='19'>&nbsp;Shops before 2019": shops_groups17,
    "<img src='assets/img/alla_bolag.png' width='15' height='19'>&nbsp;Allabolag Tobak Shops": shops_groups_alla_tobak,
...
document.getElementById('input-number-min').setAttribute("value", 5);
document.getElementById('input-number-max').setAttribute("value", 44);
...
slidervar.noUiSlider.on('update', function( values, handle ) {
    ...
    //                   +  '<p><b> Invoices Count since 24/3/2020: '+ '</b> <br>' +                feture.properties.customer_class_filtered
                ...
                function(feature, layer) {
                     return (feature.properties.avg_diff_invoices <= rangeMax) && (feature.properties.avg_diff_invoices >= rangeMin);
                },
